...
local WALKING_SPEED = 90
local JUMP_VELOCITY = 260
...
-- Flag collision left
function Player:hitFlagLeft()
    if self.dx < 0 then

        if self.map:hitsFlag(self.map:tileAt(self.x - 1, self.y)) or
            self.map:hitsFlag(self.map:tileAt(self.x - 1, self.y + self.height - 1)) then
            -- if so, reset velocity and position and change state
            self.dx = 0
            self.x = (self.map:tileAt(self.x + self.width, self.y).x - 1) * self.map.tileWidth - self.width
            love.reset()
...
-- flag collision right
function Player:hitFlagRight()
    if self.dx > 0 then

        if self.map:hitsFlag(self.map:tileAt(self.x + self.width, self.y)) or
            self.map:hitsFlag(self.map:tileAt(self.x + self.width, self.y + self.height - 1)) then
            -- if so, reset velocity and position and change state
            self.dx = 0
            self.x = (self.map:tileAt(self.x + self.width, self.y).x - 1) * self.map.tileWidth - self.width
            love.reset()
